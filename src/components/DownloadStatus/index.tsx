...
function DownloadStatus({ id }: { id: string }) {
    ...
    const { data, error, isLoading } = useSWR(
        stop ? null : "/torrent/download/status?id=" + id,
        get,
        {
        ...
        <>
            <div className="w-[100%] flex flex-col items-center gap-5">
                {stop ? (
                    ...
                    <div>Please Wait till we prepare your required file!</div>
                )}
                {!stop && (
                    <>
                ...
                <a
                    href={`${process.env.NEXT_PUBLIC_HOST}torrent/video_stream?id=${id}`}
                    download
                >
                    <Button variant="contained">
                        {stop ? "Download Link" : "Stream Link"}
                    </Button>
                </a>
            </div>
        </>
